...
def remesh(image: np.ndarray,
    ...
    I, q_y, q_z, _, _ = splitBBox.histoBBox2d(weights=image,
                                              pos0=q_h + 100,  # artificial offset to hack pyFAI to accept <0 values
                                              delta_pos0=np.ones_like(image) * (q_h_range[1] - q_h_range[0]) / bins[0],
                                              pos1=q_v - 1,
                                              delta_pos1=np.ones_like(image) * (q_v_range[1] - q_v_range[0]) / bins[1],
                                              bins=bins,
                                              pos0Range=tuple(map(lambda x: x + 100, q_h_range)),
                                              pos1Range=tuple(map(lambda x: x - 1, q_v_range)),
                                              dummy=None,
                                              delta_dummy=None,
                                              # mask=mask,
                                              # dark=dark,
...
if __name__ == '__main__':
    ...
    I = remesh(data, g, False, ai)  # img, x, y =
    test_show(I)
